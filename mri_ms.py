...
class MRI(): 
    ...
    def cal_box_corr(self): 
        ...
        if h >= 1: 
            ...
            w_sum = w_1 + w_2
            self.box_corr = (torch.sinc(w_1) * torch.sinc(w_2) * torch.sinc(w_sum))[None, None, :, :].double().to(self.device) 

        elif h < 1: 
            w_22 = - torch.fft.fftfreq(int(N/h)).repeat(int(N/h), 1)
            w_12 = torch.fft.fftfreq(int(N/h))[:, None].repeat(1, int(N/h))
            w_sum2 = w_22 + w_12
            self.box_corr  = (torch.sinc(w_12) * torch.sinc(w_22) * torch.sinc(w_sum2))[None, None, :, :].double().to(self.device)

        return

    def H(self, x): 
    
        x = F.pad(x[:, :, 1:, 1:], (1, 0, 1, 0))
        h = self.h 
        N = self.N
        s = int(np.log2(h))
        if h >= 1: 
