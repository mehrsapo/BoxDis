...
class MultiResSolverPixel():

    # only fourier measurements

    def __init__(self, forward_model='fm', reg='tv_pixel', lmbda=0., mask=None, h_init=4, range_r=512, N_scales=6,
        ...
        if  self.forward_model == 'fm':
            ...
            self.mri = MRIPixel(mask, self.device)
            ...
            self.size_scales = [self.N_init * 2**(i) - (2**i) for i in range(N_scales)]
        ...
        self.iters = list()
        self.times = list()
        self.sam = SamplerPixel(self.device) 

        if self.reg == 'htv':
            raise NotImplementedError

        if self.reg == 'tv_pixel':
            self.prox_tv_pixel = prox_tv_Pixel(device, box_const)
    ...
    def cal_cost_fm(self, y, c_k, n_scale): 
        ...
        if self.reg == 'htv':
            raise NotImplementedError
        if self.reg == 'tv_pixel':
            reg_val = (self.prox_tv_pixel.tv.L(c_k, self.h_scales[n_scale]).abs()).sum()
    ...
    def solve_sacle_fm(self, n_scale, c_k, d_k, y):
        ...
        for i in range(n_iter_out): 
            ...
            w_k = d_k + alpha * torch.real(self.mri.Ht(y - self.mri.H(d_k)))
            ...
            if self.reg == 'tv_pixel':            
                ...
                if self.lmbda > 0:
                    c_kp1 = self.prox_tv_pixel.eval(w_k, self.size_scales[n_scale], self.n_iters_in[n_scale],
