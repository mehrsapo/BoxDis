...
t0 = time.time()
for _ in range(10):
    prox_svd3d_closed(y, 200, 0.02, img_tensor.size(2), img_tensor.size(3), img_tensor.size(4), device, verbose=False)
t1 = time.time()

if device != 'cpu':
    c_svd_closed = prox_svd3d_closed(y, 200, 0.02, img_tensor.size(2), img_tensor.size(3), img_tensor.size(4), device, verbose=True)
    torch.save(c_svd_closed.cpu(), 'ExpB/3d/signal/svd_closed.pt')

if device != 'cpu':
    device = 'gpu'

np.savetxt('ExpB/3d/time/svd_closed_' + device + '.txt', np.array([(t1-t0)/10]))