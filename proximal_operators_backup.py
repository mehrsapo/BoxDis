...
def prox_svd2d(y, niter, lmbda, im_size_x, im_size_y, device, verbose=False):
    ...
    if verbose: 
        log_dir = 'loss_results/2d_htv_svd_loss.txt'
        loss_list = list()
...
def prox_svd2d_closed(y, niter, lmbda, im_size_x, im_size_y, device, verbose=False):
    ...
    if verbose: 
        log_dir = 'loss_results/2d_htv_svd_closed_loss.txt'
        loss_list = list()
...
def L_box_htv2d(x, filters): 
    ...
    Lx = F.conv2d(x, filters, padding=1)
...
def Lt_box_htv2d(y, filters):
    ...
    return F.conv_transpose2d(y, filters, padding=1)

def prox_box_htv2d(y, niter, lmbda, im_size_x, im_size_y, device, verbose=False):

    filters = torch.zeros([3, 1, 3, 3]).to(device).double()
    filters[0, ...] = 2 * torch.Tensor([[[[1., -1., 0], [-1., 1., 0], [0, 0, 0]]]]).to(device).double()
    filters[1, ...] = torch.Tensor([[[[1., 0., 0], [-1., -1., 0], [0., 1., 0]]]]).to(device).double()
    filters[2, ...] = torch.Tensor([[[[1., -1., 0.], [0., -1., 1.], [0, 0, 0]]]]).to(device).double()
    ...
    if verbose: 
        log_dir = 'loss_results/2d_htv_box_loss.txt'
        loss_list = list()
...
def prox_box3d(y, niter, lmbda, im_size_x, im_size_y, im_size_z, device, verbose=False):
    ...
    v_k = torch.zeros((1, 6,im_size_x, im_size_y, im_size_z), requires_grad=False, device=device).double()
    u_k = torch.zeros((1, 6,im_size_x, im_size_y, im_size_z), requires_grad=False, device=device).double()
    ...
    if verbose: 
        log_dir = 'loss_results/3d_htv_box_loss.txt'
        loss_list = list()
...
def schatten_proj_3d_closed(hessian, device, tol=1e-3):
...
def prox_svd3d_closed(y, niter, lmbda, im_size_x, im_size_y, im_size_z, device, verbose=False):
    ...
    if verbose: 
        log_dir = 'loss_results/3d_htv_svd_closed_loss.txt'
        loss_list = list()
...
def prox_svd3d(y, niter, lmbda, im_size_x, im_size_y, im_size_z, device, verbose=False):
    ...
    if verbose: 
        log_dir = 'loss_results/3d_htv_svd_loss.txt'
        loss_list = list()
