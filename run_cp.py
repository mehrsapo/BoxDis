...
for tau in taus:
    t0 = time.time()
    mrs_cp = MultiResSolver_cp('fm', 'htv', lmbda =1e-5, mask=mask, h_init=1, range_r = 512, N_scales=1, device=device, verbose=True, toi=-1, tau=tau, theta=1, obj_stop=True)
    mrs_cp.solve_fm(y)
    np.savetxt('compare_cp/cp_loss_theta1_tau'+str(tau)+'.txt', np.array(mrs_cp.losses[0]))
    np.savetxt('compare_cp/cp_loss_theta1_tau'+str(tau)+'_iters.txt', np.array(mrs_cp.iters))
